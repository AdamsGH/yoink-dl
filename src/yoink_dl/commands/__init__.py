"""Command handler registration for the dl plugin.

Each module exposes register(app) using the yoink-bot pattern.
...
_SKIP = {"__init__", "lang_ext", "clip", "inline"}
...
def get_handler_specs() -> list[HandlerSpec]:
    ...
    for module_info in pkgutil.iter_modules([str(_PKG_DIR)]):
        ...
        if name.startswith("_") or name in _SKIP:
            continue
        try:
            module = importlib.import_module(f"yoink_dl.commands.{name}")
            if hasattr(module, "register"):
                module.register(shim)
                logger.debug("Registered command module: %s", name)
        except Exception:
            logger.exception("Failed to register command module: %s", name)
    return shim.specs
